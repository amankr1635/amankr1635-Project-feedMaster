...
const {isValidNo,passwordVal,isValidEmail} = require("../validations/validation");
...
try {
        ...
        if(!isValidEmail(body.email))return res.status(400).send({status:false,message:"Please Enter a valid email"})
        if(!body.phone)return res.status(400).send({status:false,message:"Please Enter Your Phone No"})
        body.phone =body.phone.trim()
        if(body.phone=="")return res.status(400).send({status:false,message:"Please Enter Your Phone No"})
        if(!isValidNo(body.phone))return res.status(400).send({status:false, message:"Enter A valid Phone No"})
        if(!body.email)return res.status(400).send({status:false,message:"Please Enter Your email"})
        ...
        if(body.password=="")return res.status(400).send({status:false,message:"Please Enter Password"})
        if(!passwordVal(body.password))return res.status(400).send({status:false, message:"Password must contains at least 1 lowercase, 1 uppercase, 1 numeric character at least one special character and must be minnimun 8 character"})
        ...
        return res.status(201).send({status:true,message: "Sucessfully SignedUp" ,data:create})
} catch (error) {
...
try {
        ...
        let userCheck = await userModel.findOne({$or:[{email:body.email},{phone:body.phone}]})
